...
Deputs Step200    
	...
	Deputs "cfg enable_fcs_error_insertion"
...
Deputs "caption list:$captionList"
	...
	set pagnum [ixNet getA {::ixNet::OBJ-/statistics/view:"Traffic Item Statistics"/page} -totalPages]
	while {$sindex <= $pagnum} {
	    
		ixNet setA {::ixNet::OBJ-/statistics/view:"Traffic Item Statistics"/page} -currentPage $sindex
        ixNet commit
...
Deputs "----- TAG: $tag -----"
    ...
    set EEthType [ list ipv4 ipv6 arp mplsunicast mplsmulticast rarp pppoe_session ppp pppoe_dis ]
    set EEthTypeVal [ list 0x0800 0x86dd 0x0806 0x8847 0x8848 0x8035 0x8864 0x880b 0x8863 ]
...
Deputs "daStep:$daStep"
		  ...
		  -dst_mac_step {
			set value [ MacTrans $value ] 
			set dstmacstep $value
Deputs "dstmacstep: $dstmacstep"			 
		  }
		  -src {
			 set value [ MacTrans $value ]
			 if { [ IsMacAddress $value ] } {
...
Deputs "saStep:$saStep"
		  ...
		  -src_mac_step {
			set value [ MacTrans $value ] 
			set srcmacstep $value
Deputs "srcmacstep: $srcmacstep"			 
		  }
		  -type {
Deputs "type: $value"
...
Deputs "step:$step"
				...
				if { [ info exists daNum ] && [ info exists dstmacstep ] } {
				    set daoffset [ expr 48 - $daoffset ]
				    AddFieldMode $daReCnt
				    AddField destinationAddress
				    AddFieldConfig [ list $daoffset $da $daNum $dstmacstep ]
				}
			 }
		  }
	   }
...
Deputs "step:$step"
				...
				if { [ info exists saNum ] && [ info exists srcmacstep ] } {
				    set saoffset [ expr 48 - $saoffset ]
				    AddFieldMode $saReCnt
				    AddField sourceAddress
				    AddFieldConfig [ list $saoffset $sa $saNum $srcmacstep ]
				} 
